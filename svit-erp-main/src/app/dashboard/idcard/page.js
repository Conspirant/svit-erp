...
export default function IDCardPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (cached) {
        ...
        const dashData = sessionStorage.getItem("dashboard_data");
        if (dashData) {
      ...
      try {
        ...
        if (profileJson.success && profileJson.data) {
          setProfile((prev) => ({
            ...
            usn: profileJson.data.usn || prev?.usn || "",
            department: profileJson.data.department || prev?.department || "",
          }));
          try {
        ...
        console.warn("Could not connect to ERP profile:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error && !profile && !savedStudentId) {
    return (
      <div className="center-state">
